...
import {getExerciseTitles, postNewTraining, getExercisesByTrainingPk, getSetsByExercisePk} from "@/services/defaultService.js"
    ...
    export default {
        ...
        methods: {
            async updateEvent() {
                this.trainingTitle = this.selectedEvent.name
                this.startDate = this.selectedEvent.start.slice(0, 10)
                this.startTime = this.selectedEvent.start.slice(11, 16)
                this.exercises = []
                var exercises = await getExercisesByTrainingPk(this.selectedEvent.pk)
                for(const exercise of exercises){
                  var newExercise = {
                    id: exercise.pk,
                    title: this.exercisesTypes[exercise.fields.title],
                    sets: [],
                  }

                  var sets = await getSetsByExercisePk(exercise.pk)
                  sets.forEach(set =>{
                    newExercise.sets.push({
                      id: set.pk,
                      weight: set.fields.weight,
                      reps: set.fields.reps,
                    })
                  })
            ...
            async updateExerciseTitles() {
              ...
              dataFromServer.forEach(element => {
                this.exercisesTypes[element.pk] = element.fields.title
              });
            },
            async acceptForm() {
              var data = {
